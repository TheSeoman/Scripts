...
id.map <-
  covariates.all[!is.na(covariates.all$expr_s4f4ogtt) &
                   !is.na(covariates.all$axio_s4f4), c('axio_s4f4', 'expr_s4f4ogtt')]
id.map <- id.map[order(id.map$expr_s4f4ogtt), ]

covariates.expr <-
  covariates.all[!is.na(covariates.all$expr_s4f4ogtt), c('ucsex', 'utalteru', 'utbmi', 'ul_wbc', 'expr_s4f4ogtt')]
...
indices.snp <-
  as.vector(sample.index.map[as.character(id.map$axio_s4f4), 2])
