...
names(blood.tfbs.ranges) <-
  paste0(
    blood.tfbs.ranges$TF,
    '_',
    seqnames(blood.tfbs.ranges),
    ':',
    start(blood.tfbs.ranges),
    '-',
    end(blood.tfbs.ranges)
  )


save(blood.tfbs.ranges, file = PATHS$TFBS.RANGES.DATA)
