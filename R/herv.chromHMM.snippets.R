...
write.table(annotation,
            file=file.path(PATHS$CHROMHMM.OUT.DIR, "cpgs_with_epigenetic_states.txt"),
            sep="\t",
            quote=F)
...
pdf(file=file.path(PATHS$CHROMHMM.OUT.DIR, "epigenetic_state_annotation.pdf"))
print(qplot(State, Cell, fill=Proportion, geom="tile", data=plot.data)
      +  theme(axis.text.x=element_text(angle=90, hjust = 1, vjust=0.5))
      + facet_grid(type ~ ., scale="free_y", space="free_y"))
dev.off()
...
write.table(gran,
            file=file.path(PATHS$CHROMHMM.OUT.DIR,
...
pdf(file=file.path(PATHS$CHROMHMM.OUT.DIR,
...
pdf(file=file.path(PATHS$CHROMHMM.OUT.DIR, "houseman_boxplot_kora.pdf"))
...
pdf(file=file.path(PATHS$CHROMHMM.OUT.DIR, "epigenetic_state_annotation_weighted.pdf"))
...
chromHMM.range.annotation.proportions <- function (ranges, ids, dir = paste0(PATHS$ROADMAP.DIR, "chromHMM/15state/"), suffix = "_15_coreMarks_mnemonics.bed.bgz") {
  library(Rsamtools)
